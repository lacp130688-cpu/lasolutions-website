      ...
      <head>
        <Script
          ...
          dangerouslySetInnerHTML={{
            __html: `console.log('CLEANUP_START', Date.now());(function(){try{var m=document.querySelectorAll('meta[name="hosting-provider"],meta[name="netlify-deploy"]');for(var i=0;i<m.length;i++){m[i].parentNode.removeChild(m[i]);}var n=document.head.childNodes;for(var i=n.length-1;i>=0;i--){if(n[i].nodeType===8&&n[i].textContent.indexOf('Netlify')!==-1)n[i].parentNode.removeChild(n[i]);}}catch(e){console.log('CLEANUP_ERR', e.message)}})();console.log('CLEANUP_END', Date.now());`,
          }}
        />
